# modules/notify.py (FINAL REVIEWED CODE - MODIFIED)
...
def send_telegram_message(message: str):
    ...
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID: # <-- 변경된 부분: TELEGRAM_TOKEN -> TELEGRAM_BOT_TOKEN
        logger.warning("⚠️ 텔레그램 토큰 또는 채팅 ID가 설정되지 않아 메시지를 보낼 수 없습니다. config.py 또는 관련 환경 변수를 확인하세요.")
        return

    try:
        url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage" # <-- 변경된 부분: TELEGRAM_TOKEN -> TELEGRAM_BOT_TOKEN
        payload = {
...
if __name__ == "__main____":
    ...
    # 경고: 실제 실행 시 config.py에서 TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID가 로드되지 않으면 테스트 실패
