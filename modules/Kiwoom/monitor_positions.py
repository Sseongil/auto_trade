...
class MonitorPositions:
    ...
    def update_position(self, stock_code, quantity, purchase_price=0):
        ...
        with self.position_lock:        
            ...
            current_total = position["total_purchase_amount"]
            current_price = position["purchase_price"]
        
            if quantity > 0:  # 매수
                new_total = current_total + (quantity * purchase_price)
                new_qty = current_qty + quantity
                avg_price = new_total / new_qty if new_qty > 0 else 0.0

                position["quantity"] = new_qty
                position["total_purchase_amount"] = new_total
                position["purchase_price"] = avg_price

                # 최초 매수인 경우 buy_time 설정
                if "buy_time" not in position or not position["buy_time"]:
                    position["buy_time"] = now_str
                    position["buy_date"] = today_str

                if position["trail_high"] == 0.0 or purchase_price > position["trail_high"]:
                    position["trail_high"] = purchase_price

                logger.info(f"{get_current_time_str()}: Updated position for {stock_code}: Buy {quantity} @ {purchase_price}. New Avg: {avg_price:.2f}, Total Qty: {new_qty}")
                ...
                if current_qty >= sell_qty:
                    ...
                    if current_qty > 0:
                        unit_price = current_total / current_qty
                        position["total_purchase_amount"] -= unit_price * sell_qty

                    position["quantity"] = new_qty

                    if new_qty == 0:
                        position["purchase_price"] = 0.0
                        position["total_purchase_amount"] = 0.0
                        position["trail_high"] = 0.0
                        position["half_exited"] = False
                        position["buy_time"] = None
                        position["buy_date"] = None
                    elif position["total_purchase_amount"] < 0:
                        position["total_purchase_amount"] = 0.0

                    logger.info(f"{get_current_time_str()}: Updated position for {stock_code}: Sell {sell_qty}. New Qty: {new_qty}")
                else:
                    logger.warning(f"{get_current_time_str()}: Attempted to sell {sell_qty}, but only {current_qty} held.")
                    return False

            position["last_update"] = now_str
            self.save_positions()
            return True
    ...
    def remove_position(self, stock_code):
        with self.position_lock:
            if stock_code in self.positions:
                ...
                logger.info(f"{get_current_time_str()}: Position removed: {stock_code}")
                self.save_positions()
